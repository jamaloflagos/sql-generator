...
const Select = () => {
  ...
  const handleSubmit = () => {
    ...
    if (!tableName || !columns.length > 0) {
        setMessage('You did not fill table name or columns');
        return
    }
  ...
  return (
    <div>
      <h2>Generate SELECT Statement</h2>
      <div>
  ...
  <div>
    ...
    <select
      ...
      onChange={(e) => setJoinInput({ ...joinInput, type: e.target.value })}
    >
    ...
    <input
      ...
      onChange={(e) => setJoinInput({ ...joinInput, table: e.target.value })}
    />
    <label>Alias:</label>
    <input
      type="text"
      value={joinInput.alias}
      onChange={(e) => setJoinInput({ ...joinInput, alias: e.target.value })}
    />
    <label>Condition:</label>
    <input
      type="text"
      value={joinInput.condition}
      onChange={(e) => setJoinInput({ ...joinInput, condition: e.target.value })}
    />
  ...
  <ul>
    {joins.map((join, index) => (
      <li key={index}>
        {join.type} JOIN {join.table} {join.alias && 'AS'} {join.alias} ON {join.condition} 
    ...
    <div>
        ...
        <select value={aggregateInput.function} onChange={e => setAggregateInput({ ...aggregateInput, function: e.target.value })}>
        ...
        {!selectAllColumns && (
            <div>
                ...
                <input
                    ...
                    onChange={e => setAggregateInput({ ...aggregateInput, column: e.target.value })}
                />
            </div>
        )}
        <label>Alias:</label>
        <input
            ...
            onChange={e => setAggregateInput({ ...aggregateInput, alias: e.target.value })}
        />
    ...
    <ul>
        {aggregates.map((aggregate, index) => (
            <li key={index}>
                {aggregate.function}({aggregate.column || '*'}) {aggregate.alias && 'AS'} {aggregate.alias} 
      ...
      <div>
        ...
        <div>
          <label>
            ...
            />
            <select name="option" value={clauses.fetch.option} onChange={handleFetchChange} disabled={!clauses.fetch.status}>
